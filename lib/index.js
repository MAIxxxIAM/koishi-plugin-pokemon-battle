...
async function apply(ctx, config) {
    ...
    ctx.before('send', async (session) => {
        if (session._stripped.content !== session._stripped.prefix + '宝可梦' || session._sendType !== 'command')
            return;
        ...
        const bgImg = await ctx.canvas.loadImage((0, path_1.resolve)(__dirname, './qiandao', 'pokemonbg.png')); //上下空余11px,左右空余8px
    ...
    ctx.command('宝可梦').subcommand('宝可梦签到', '每日的宝可梦签到')
        ...
        if (userArr.length != 0) {
            ...
            `;
                ...
                //图片服务
                let image = await ctx.canvas.loadImage((0, path_1.resolve)(__dirname, './qiandao', '签到.png'));
                ...
                let expbar = await ctx.canvas.loadImage((0, path_1.resolve)(__dirname, './qiandao', 'expbar.png'));
                let overlay = await ctx.canvas.loadImage((0, path_1.resolve)(__dirname, './qiandao', 'overlay_exp.png'));
                let time = Date.now();
            ...
            //图片服务
            const bg_img = await ctx.canvas.loadImage((0, path_1.resolve)(__dirname, './qiandao', 'spawn.png'));
            const pokemonimg = await ctx.canvas.loadImage((0, path_1.resolve)(__dirname, `./images/${firstMonster_}.png`));
    ...
    ctx.command('宝可梦').subcommand('捕捉宝可梦', '随机遇到3个宝可梦')
        ...
        if (userArr.length == 0) {
            ...
            if (userArr[0].captureTimes > 0) {
                ...
                let poke_img = [];
                let dataUrl;
                let bg_img = await ctx.canvas.loadImage((0, path_1.resolve)(__dirname, './qiandao', 'catchBG.png'));
                poke_img[0] = await ctx.canvas.loadImage((0, path_1.resolve)(__dirname, './images', grassMonster[0] + '.png'));
                poke_img[1] = await ctx.canvas.loadImage((0, path_1.resolve)(__dirname, './images', grassMonster[1] + '.png'));
                poke_img[2] = await ctx.canvas.loadImage((0, path_1.resolve)(__dirname, './images', grassMonster[2] + '.png'));
                let grassImg = await ctx.canvas.loadImage((0, path_1.resolve)(__dirname, './qiandao', 'Grass.png'));
...
${((0, koishi_1.h)('at', { id: (session.userId) }))}
    ...
    ctx.command('宝可梦').subcommand('杂交宝可梦', '选择两只宝可梦杂交')
        ...
        if (userArr.length != 0) {
            //图片服务
            let pokemonimg1 = [];
            const bgImg = await ctx.canvas.loadImage((0, path_1.resolve)(__dirname, './qiandao', 'bag.png'));
            for (let i = 0; i < userArr[0].AllMonster.length; i++) {
                pokemonimg1[i] = await ctx.canvas.loadImage((0, path_1.resolve)(__dirname, `./images/${userArr[0].AllMonster[i].split('.')[0]}.png`));
            }
            const image = await ctx.canvas.render(512, 381, ctx => {
...
`);
            ...
            if (zajiao) {
                ...
                if (dan == 0 || dan[0] == 0) {
                    ...
                    if (userArr[0].monster_1 != '0') {
                        //图片服务
                        let img_fuse = await ctx.canvas.loadImage((0, path_1.resolve)(__dirname, './qiandao/fuse.png'));
...
${((0, koishi_1.h)('at', { id: (session.userId) }))}`;
    ...
    ctx.command('宝可梦').subcommand('查看信息 <user:string>', '查看用户信息')
        ...
        let userId;
        let infoImgSelf;
        const infoImgSelf_bg = await ctx.canvas.loadImage((0, path_1.resolve)(__dirname, './qiandao', 'trainercard.png'));
        let expbar = await ctx.canvas.loadImage((0, path_1.resolve)(__dirname, './qiandao', 'expbar.png'));
        let overlay = await ctx.canvas.loadImage((0, path_1.resolve)(__dirname, './qiandao', 'overlay_exp.png'));
        if (!user) {
            //查看自己信息
        ...
        try {
            ...
            if (!tarArr[0].relex) {
                await ctx.database.set('pokebattle', { id: userId }, {
                    relex: new Date(battlenow - 3600000 * 2)
                });
            }
            const getTimes = (battlenow - tarArr[0].relex.getTime()) / 3600000 > 3 ? 3 : Math.floor((battlenow - tarArr[0].relex.getTime()) / 3600000);
            ...
            tarArr[0].battleTimes = (getTimes + tarArr[0].battleTimes - 1) >= 2 ? 2 : getTimes + tarArr[0].battleTimes - 1;
            tarArr[0].relex = (battlenow - tarArr[0].relex.getTime()) / 3600000 > 3 ? new Date(battlenow) : new Date(3600000 * getTimes + tarArr[0].relex.getTime());
            if (tarArr[0].battleTimes < 0) {
                tarArr[0].battleTimes = 0;
                return `对方的宝可梦还在恢复，无法对战`;
            }
            await ctx.database.set('pokebattle', { id: userId }, {
                battleTimes: tarArr[0].battleTimes,
                relex: tarArr[0].relex
            });
            ...
            tarArr[0].power = pokemon_1.default.power(pokemon_1.default.pokeBase(tarArr[0].monster_1), userArr[0].level);
