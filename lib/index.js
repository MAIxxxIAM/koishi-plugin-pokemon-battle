...
exports.using = ['database', 'echarts'];
exports.usage = `### 10.20 新增功能
- 更明显的经验条显示
- 放生获得经验
- 指令回复更合理的产生联动
- 【属性】指令的雷达图样式
...
`);
            ...
            if (zajiao) {
                ...
                let tip = '';
                if (pokeM == '150.150' && pokeW == '150.150')
                    tip = '超梦是人造宝可梦，自交会导致基因断裂，成为梦幻形态';
...
${(toDo)}
    ...
    ctx.command('对战 <user:user>')
        ...
        try {
            ...
            if (userArr[0].gold < 500 || tarArr[0].battleTimes == 0) {
                return (`你的金币不足或者对方的宝可梦还在恢复，无法对战`);
            }
            else if (session.userId == user) {
                return (`你不能对自己发动对战`);
            }
            else if (!tarArr[0].length || tarArr[0].monster_1 == '0') {
                return (`对方还没有宝可梦`);
            }
            session.send(`你支付了500金币，对${((0, koishi_1.h)('at', { id: (userId) }))}发动了宝可梦对战`);
            ...
            if (tarArr[0].battleTimes == 1) {
                ...
                session.send(`${(0, koishi_1.h)('at', { id: (userId) })}你的宝可梦已经筋疲力尽，2小时后恢复完毕`);
            }
...
`;
    ...
    ctx.command('装备技能 <skill>')
        ...
        console.info(userArr[0].skillbag.includes(String(pokemon_1.default.findskillId(skill))));
        await ctx.database.set('pokebattle', { id: session.userId }, {
